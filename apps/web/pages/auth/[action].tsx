...
export const getStaticProps: GetStaticProps = async ({ locale, params }) => {
  ...
  if (!action || !authActions.includes(action)) return { redirect: '/', props: {} };
...
export const getStaticPaths: GetStaticPaths = ({ locales }) => ({
  paths: locales?.map((locale) => authActions.map((action) => ({ params: { action }, locale }))).flat() ?? [],
  fallback: true,
});
            ...
            <VStack align="stretch" spacing={12}>
              ...
              <HStack justify="space-between">
                <Text py={1} color="gray.500">
                  © {new Date().getFullYear()} The Rogue Scholar. {t('footer.allRightsReserved', { ns: 'common' })}
                </Text>
              </HStack>
            </VStack>
          </Container>
