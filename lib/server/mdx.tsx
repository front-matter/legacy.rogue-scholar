...
export async function getMdxStaticProps(
  ...
  const fileContent = await readMdxFile(
    dir,
    slug,
    locale !== defaultLocale ? locale : undefined
  )
...
export async function getMdxStaticPaths(
  ctx: Pick<GetStaticPathsContext, "locales" | "defaultLocale">,
  dir: string
) {
  const paths: Array<{
    params: any
